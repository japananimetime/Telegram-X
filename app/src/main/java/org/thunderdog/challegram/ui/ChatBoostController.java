...
public class ChatBoostController extends RecyclerViewController<ChatBoostController.Args> implements View.OnClickListener {
  ...
  // Slots the user can apply to this chat right now: not already boosting it, and
  // not still in cooldown (a slot reassigned from another chat can't move again until
  // cooldownUntilDate). Free slots (boosting nothing) are ordered ahead of occupied
  // ones so we don't steal a boost from another chat while an unused slot exists.
  private List<TdApi.ChatBoostSlot> applicableSlots () {
    List<TdApi.ChatBoostSlot> free = new ArrayList<>();
    List<TdApi.ChatBoostSlot> occupied = new ArrayList<>();
    if (slots != null) {
      int now = (int) tdlib.currentTime(java.util.concurrent.TimeUnit.SECONDS);
      for (TdApi.ChatBoostSlot slot : slots) {
        if (slot.currentlyBoostedChatId == chatId) {
          continue; // Already boosting this chat with this slot.
        }
        if (slot.cooldownUntilDate > now) {
          continue; // Still cooling down; can't be reassigned yet.
        }
