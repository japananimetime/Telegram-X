...
public class StarRevenueController extends RecyclerViewController<StarRevenueController.Args> implements View.OnClickListener {
  ...
  private static double toStars (TdApi.StarAmount amount) {
    return amount == null ? 0 : amount.starCount + amount.nanostarCount / 1_000_000_000.0;
  }
  ...
  private String usd (TdApi.StarAmount amount) {
    ...
    double dollars = toStars(amount) * statistics.usdRate;
    return String.format(java.util.Locale.US, " · ≈ $%.2f", dollars);
  }

  private static CharSequence starsText (TdApi.StarAmount amount) {
    if (amount == null || amount.nanostarCount == 0) {
      return Lang.plural(R.string.xStars, amount != null ? amount.starCount : 0);
    }
  ...
  private void addAmountRow (List<ListItem> items, int titleRes, TdApi.StarAmount amount, boolean first) {
    ...
    item.setData(starsText(amount) + usd(amount));
    items.add(item);
  }
