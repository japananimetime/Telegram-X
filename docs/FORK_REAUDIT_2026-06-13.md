...
| 9 | **Group calls / video chats / live streams** (XL — being sliced) — **slice 1 ✅ DONE** (`e89f5f8af`): read-only `GroupCallController` (`GetGroupCall` + `GroupCallListener` now has an implementer + subscribe helpers); `openVoiceChat` wired (was a stub), so video-chat service messages are tappable. **Remaining:** slice 2 join/leave/mute (WebRTC pipeline), slice 3 scheduling/live-stream/RTMP, full participant roster, in-chat call bar. | High | XL |
