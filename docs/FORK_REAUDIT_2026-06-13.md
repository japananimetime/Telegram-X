...
| 9 | **Group calls / video chats / live streams** (XL — being sliced) — **slice 1 ✅** (`e89f5f8af`): read-only `GroupCallController` (`GetGroupCall` + `GroupCallListener` implementer + subscribe helpers); `openVoiceChat` wired. **slice 1.5 ✅** (`072705b93`): in-chat active video-chat bar at the top of the chat (`chat.videoChat.groupCallId`), live type label via `GroupCallListener`, tap → read-only screen. **Remaining:** slice 2 join/leave/mute (WebRTC pipeline), slice 3 scheduling/live-stream/RTMP, full participant roster. | High | XL |
