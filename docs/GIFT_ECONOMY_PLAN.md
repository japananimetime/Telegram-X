...
- **Slice 3 — Send gift + settings** (IN PROGRESS). `GetAvailableGifts`/`SendGift`/`CanSendGift`/`SetGiftSettings`/`SellGift`; gift-picker controller; "Send a gift" entry in `ProfileController.showPrivateMore()`. Needs real star purchase (`SettingsStarsController.purchaseStars` is stubbed).
